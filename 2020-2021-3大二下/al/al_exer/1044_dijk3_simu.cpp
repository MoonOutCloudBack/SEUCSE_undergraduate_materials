...
inline void findpath(int i,int j,int num,int &times){
    int k,pre;
    if(times>TIMES&&!out[num].empty()){
        pre=out[num][out[num].size()-1];
        for(int u=out[num].size()-3;u>=0;u-=2)
            k=out[num][u],e[k][pre].t0-=e[k][pre].alpha*0.8,pre=k;
    }
    out[num].clear();
    k=path[j],pre=j;out[num].push_back(j);
    while(k!=-1){
        out[num].push_back(e[k][pre].num),out[num].push_back(k);
        e[k][pre].t0+=e[k][pre].alpha*0.8;
        pre=k,k=path[k];
    }
}
