...
inline void clearpath(int num){
    int k,pre;
    if(!out[num].empty()&&out[num].size()>2){
        pre=out[num][out[num].size()-1];
        for(int u=out[num].size()-3;u>=0;u-=2)
            k=out[num][u],e[k][pre].t0-=e[k][pre].alpha,pre=k;
    }
}
...
int main(){
    ...
    cin>>T;while(T--){
        ...
        for(int i=0;i<E;++i){
            ...
            e[u][v].alpha=temp2*temp0;
            e[v][u].alpha=temp1*temp0;
        ...
        for(int times=0;times<TIMES;++times){
            ...
            for(int des=0;des<m;++des){
                ...
                if(base>=limit||base==n)continue;

                for(int i=base;i<limit;++i)clearpath(nav[i].num);
